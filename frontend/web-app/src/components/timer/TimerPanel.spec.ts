...
describe('personal cross-project timer picker', () => {
  ...
  it('swaps the time for the play/pause control inside the orb while hovered and keeps the capsule for context', async () => {
    tracker.current.value = { session: { id: 's', workItemId: 'a' }, workItemTitle: '工作a', recentItems: [], rowVersion: 1 } as never
    const wrapper = await panel(false)
    expect(wrapper.get('.orb-shell').classes()).not.toContain('is-control')
    expect(wrapper.get('.orb-disc').text()).toContain('02:00')
    expect(wrapper.get('.orb-disc').text()).not.toContain('工作a')
    await wrapper.get('.orb-layout').trigger('pointerenter')
    expect(wrapper.get('.orb-shell').classes()).toContain('is-control')
    expect(wrapper.find('.orb-capsule').exists()).toBe(false)
    await wrapper.get('.orb-disc [aria-label="暂停计时"]').trigger('click')
    expect(tracker.stop).toHaveBeenCalledOnce()
    await vi.advanceTimersByTimeAsync(360)
    expect(wrapper.get('.capsule-title').text()).toBe('工作a')
    expect(wrapper.get('.capsule-time').text()).toBe('0:02:00')
    expect(wrapper.get('.orb-capsule').findAll('button').map(button => button.attributes('aria-label'))).toEqual([undefined, '更换工作'])
    await wrapper.get('.orb-capsule [aria-label="更换工作"]').trigger('click')
    expect(wrapper.find('input[role="combobox"]').exists()).toBe(true)
  })
  it('collapses an open capsule as soon as the orb is dragged, on the desktop and in the page', async () => {
    let dragging!: (value: boolean) => void
    const bridge = desktop({}, { onDragging: (listener: typeof dragging) => { dragging = listener; return vi.fn() } })
    const wrapper = await panel(false)
    bridge.hover(true); await vi.advanceTimersByTimeAsync(360)
    expect(wrapper.find('.orb-capsule').exists()).toBe(true)
    dragging(true); await vi.advanceTimersByTimeAsync(300); await flushPromises()
    expect(wrapper.find('.orb-capsule').exists()).toBe(false)
    expect(bridge.setOrbLayout).toHaveBeenLastCalledWith({ details: false })
    expect(wrapper.emitted('drag')).toBeUndefined()
    wrappers.splice(0).forEach(mounted => mounted.unmount())
    vi.stubGlobal('yumpooDesktop', undefined)
    const page = await panel(false)
    await page.get('.orb-layout').trigger('pointerenter'); await vi.advanceTimersByTimeAsync(360)
    expect(page.find('.orb-capsule').exists()).toBe(true)
    await page.get('.orb-disc').trigger('pointerdown', { button: 0 }); await vi.advanceTimersByTimeAsync(300); await flushPromises()
    expect(page.find('.orb-capsule').exists()).toBe(false)
    expect(page.emitted('drag')).toHaveLength(1)
    await vi.advanceTimersByTimeAsync(1000)
    expect(page.find('.orb-capsule').exists()).toBe(false)
  })
  it('shows the start control instead of a clock when there is nothing to time and reveals the capsule on keyboard focus', async () => {
    const wrapper = await panel(false)
    expect(wrapper.get('.orb-shell').classes()).toContain('is-control')
    expect(wrapper.find('.orb-face .timer-digits').exists()).toBe(false)
    await wrapper.get('.orb-control').trigger('focus'); await flushPromises()
    expect(wrapper.get('.capsule-title').text()).toBe('尚未选择工作项')
    await wrapper.get('[aria-label="查找工作项"]').trigger('click')
    expect(wrapper.find('input[role="combobox"]').exists()).toBe(true)
  })
  it('hands a press that travels on the orb control to the shell as a window drag instead of a click', async () => {
    tracker.current.value = { session: { id: 's', workItemId: 'a' }, workItemTitle: '工作a', recentItems: [], rowVersion: 1 } as never
    let dragging!: (value: boolean) => void
    const dragWindow = vi.fn(async () => undefined)
    const bridge = desktop({}, { dragWindow, onDragging: (listener: typeof dragging) => { dragging = listener; return vi.fn() } })
    const wrapper = await panel(false)
    bridge.hover(true); await flushPromises()
    const control = wrapper.get('.orb-control')
    await control.trigger('pointerdown', { button: 0, pointerId: 7, screenX: 100, screenY: 100 })
    await control.trigger('pointermove', { pointerId: 7, screenX: 102, screenY: 101 })
    expect(dragWindow).not.toHaveBeenCalled()
    await control.trigger('pointermove', { pointerId: 7, screenX: 110, screenY: 104 })
    expect(dragWindow).toHaveBeenCalledWith(true)
    dragging(true); await flushPromises()
    expect(wrapper.get('.orb-shell').classes()).toEqual(expect.arrayContaining(['is-dragging']))
    expect(wrapper.get('.orb-shell').classes()).not.toContain('is-control')
    await control.trigger('pointerup', { pointerId: 7 })
    await control.trigger('click')
    expect(dragWindow).toHaveBeenLastCalledWith(false)
    expect(tracker.stop).not.toHaveBeenCalled()
    bridge.hover(true); await vi.advanceTimersByTimeAsync(400)
    expect(bridge.setOrbLayout).not.toHaveBeenCalled()
    dragging(false); await flushPromises()
    await control.trigger('pointerdown', { button: 0, pointerId: 8, screenX: 100, screenY: 100 })
    await control.trigger('pointerup', { pointerId: 8 })
    await control.trigger('click')
    expect(tracker.stop).toHaveBeenCalledOnce()
  })
  ...
  it('opens the web capsule toward the side with more viewport room', async () => {
    ...
    await wrapper.get('.orb-control').trigger('focus')
    expect(wrapper.emitted('orb')?.at(-1)?.[0]).toMatchObject({ side: 'left', detailWidth: 216 })
    await wrapper.get('.orb-layout').trigger('pointerleave'); await vi.advanceTimersByTimeAsync(300); await flushPromises()
  ...
  it('renders the edge dock and slides its card out from native hover', async () => {
    ...
    expect(wrapper.get('.dock-tab').text()).toBe('2分00秒')
    bridge.hover(true); await vi.advanceTimersByTimeAsync(360)
