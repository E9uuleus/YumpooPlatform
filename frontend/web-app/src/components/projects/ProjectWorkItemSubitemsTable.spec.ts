...
describe('项目工作项子表格', () => {
  ...
  it('在子项下方插入同级草稿，失焦后创建并在当前父项内定位', async () => {
    ...
    wrapper.findComponent({ name: 'WorkItemRowActions' }).vm.$emit('createBelow', item('child-1'))
    await flushPromises()
    await wrapper.get('input.work-item-name-input').setValue('新同级子项')
    await wrapper.get('input.work-item-name-input').trigger('blur')
    await flushPromises()
  ...
  it('编辑子项名称时暂停行拖动，保存后通知主表更新', async () => {
    ...
    await row.get('input.work-item-name-input').setValue('修改子项名称')
    await row.get('input.work-item-name-input').trigger('blur')
    await flushPromises()
