...
function relationPage(items: WorkItemRelation[] = [relation()]) {
  return { items, page: 0, size: 20, totalElements: items.length, totalPages: 1, canCreate: true }
}
...
describe('WorkItemRelations', () => {
  ...
  it('展示禁用原因并在重复创建响应时不追加本地关系', async () => {
    ...
    const wrapper = mount(WorkItemRelations, {
      props: { workItemId: 'item-1' }, global: { stubs: { InlineProblem: true } },
    })
    await flushPromises()
