...
describe('外观运行时', () => {
  ...
  it('其他窗口修改外观后同步应用，并忽略非法值和无关键', () => {
    installMatchMedia(false)
    initializeAppearance()
    window.dispatchEvent(new StorageEvent('storage', { key: THEME_STORAGE_KEY, newValue: 'night' }))
    window.dispatchEvent(new StorageEvent('storage', { key: DENSITY_STORAGE_KEY, newValue: 'compact' }))
    expect(document.documentElement.dataset.theme).toBe('night')
    expect(document.documentElement.dataset.density).toBe('compact')
    ...
    expect(useAppearance().themeMode.value).toBe('night')
    resetAppearanceForTests()
