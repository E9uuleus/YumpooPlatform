...
async function triggerSync(): Promise<void> {
  ...
  try {
    ...
    const uiError = await toUiError(reason)
    if (uiError.status === 409 && uiError.location) {
      const runId = uiError.location.split('/').at(-1)
      if (runId) {
        await openRun(runId)
        return
      }
...
<template>
  <section>
    <div class="toolbar">
      <div class="filters">
        <select v-model="status" class="native-control" aria-label="运行状态" @change="applyFilters">
          <option :value="undefined">全部状态</option>
          <option v-for="value in DirectorySyncRunStatus" :key="value" :value="value">{{ value }}</option>
        </select>
        <select v-model="triggerType" class="native-control" aria-label="触发方式" @change="applyFilters">
          <option :value="undefined">全部触发方式</option>
          <option v-for="value in DirectorySyncTriggerType" :key="value" :value="value">{{ value }}</option>
        </select>
      </div>
      <el-button v-if="canWrite" type="primary" :loading="triggering" @click="triggerSync">
        立即同步
      </el-button>
    </div>
    ...
    <el-table v-if="result?.items.length" v-loading="loading" :data="result.items" stripe>
      ...
      <el-table-column label="发现/失败" width="130">
        <template #default="scope">{{ scope.row.counts.discovered }} / {{ scope.row.counts.failed }}</template>
      </el-table-column>
      <el-table-column prop="errorCode" label="错误码" min-width="170" />
      <el-table-column label="操作" width="90">
        <template #default="scope"><el-button link type="primary" @click="openRun(scope.row.runId)">详情</el-button></template>
      </el-table-column>
    </el-table>
    <el-empty v-else-if="!loading" description="暂无同步运行" />
    ...
    <el-drawer v-model="drawerOpen" title="同步运行详情" size="620px" @closed="clearPoll">
      <el-descriptions v-if="selected" :column="2" border>
        ...
        <el-descriptions-item label="触发方式">{{ selected.triggerType }}</el-descriptions-item>
        <el-descriptions-item label="触发人">{{ selected.triggeredByDisplayName ?? '系统' }}</el-descriptions-item>
        <el-descriptions-item label="开始时间">{{ formatTime(selected.startedAt) }}</el-descriptions-item>
        <el-descriptions-item label="结束时间">{{ formatTime(selected.finishedAt) }}</el-descriptions-item>
        <el-descriptions-item label="错误码">{{ selected.errorCode ?? '无' }}</el-descriptions-item>
        <el-descriptions-item label="requestId">{{ selected.requestId }}</el-descriptions-item>
        <el-descriptions-item label="安全摘要" :span="2">{{ selected.errorSummary ?? '无' }}</el-descriptions-item>
      </el-descriptions>
      <h3>失败项</h3>
      <el-table v-if="failures?.items.length" :data="failures.items" size="small">
        <el-table-column prop="maskedMemberReference" label="成员引用" />
        <el-table-column prop="result" label="结果" width="120" />
        <el-table-column prop="errorCode" label="错误码" min-width="180" />
      </el-table>
      <el-empty v-else description="无失败项" :image-size="64" />
      <el-pagination
