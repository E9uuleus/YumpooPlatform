...
describe('项目级工作项首页', () => {
  ...
  it('文字编辑保存后同步主表名称，单元格空白不打开详情', async () => {
    ...
    expect(wrapper.find('input.work-item-name-input').exists()).toBe(false)
    await wrapper.get('.work-item-title-text').trigger('click')
    await wrapper.get('input.work-item-name-input').setValue('已修改的名称')
    await wrapper.get('input.work-item-name-input').trigger('blur')
    await flushPromises()
  ...
  it('在指定行下插入空白草稿，输入后外部点击才创建并按返回版本定位', async () => {
    ...
    expect(rows[1]!.get<HTMLInputElement>('input.work-item-name-input').element.value).toBe('')
    expect(rows[1]!.get('input.work-item-name-input').attributes('placeholder')).toBe('*新工作项')
    ...
    await rows[1]!.get('input.work-item-name-input').setValue('下方新行')
  ...
  it('草稿可取消；创建失败保留名称且重复失焦不产生并发创建', async () => {
    ...
    await flushPromises()
    await wrapper.get('input.work-item-name-input').trigger('keydown', { key: 'Escape' })
    expect(wrapper.find('.work-item-draft-row').exists()).toBe(false)
    expect(state.createWorkItem).not.toHaveBeenCalled()
    menu.vm.$emit('createBelow', item())
    await flushPromises()
    ...
    const input = wrapper.get<HTMLInputElement>('input.work-item-name-input')
  ...
  it('创建成功但定位失败时仍结束草稿，避免重复创建', async () => {
    ...
    await flushPromises()
    await wrapper.get('input.work-item-name-input').setValue('定位失败仍保留')
    await wrapper.get('input.work-item-name-input').trigger('blur')
    await flushPromises()
    expect(state.createWorkItem).toHaveBeenCalledTimes(1)
  ...
  it('创建请求未返回时切换项目，不把草稿或迟到的新行插入另一个项目', async () => {
    ...
    wrapper.findComponent({ name: 'WorkItemRowActions' }).vm.$emit('createBelow', item())
    await flushPromises()
    await wrapper.get('input.work-item-name-input').setValue('切换前提交')
    await wrapper.get('input.work-item-name-input').trigger('blur')
