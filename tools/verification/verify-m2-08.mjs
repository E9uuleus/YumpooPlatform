...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m208-'))
...
const baseRef=process.env.YUMPOO_M208_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-08:assets'],{cwd:root})
  ...
  console.log('M2-08 Project 生命周期治理与 Workspace 迁移完整门禁已通过。')
