...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m215-'))
...
const baseRef=process.env.YUMPOO_M215_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-15:assets'],{cwd:root})
  ...
  console.log('M2-15 Work Item 软删除、恢复与归档只读完整门禁已通过。')
