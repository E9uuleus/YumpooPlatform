...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m214-'))
...
const baseRef=process.env.YUMPOO_M214_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-14:assets'],{cwd:root})
  ...
  console.log('M2-14 Kanban rank 与受控拖动完整门禁已通过。')
