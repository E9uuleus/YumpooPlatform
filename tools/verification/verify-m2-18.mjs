...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m218-'))
...
const baseRef = process.env.YUMPOO_M218_BASE_REF ?? 'origin/dev'

try {
  runPnpmSync(['run', 'validate:event-contracts'], { cwd: root })
  runPnpmSync(['run', 'doc-sync'], { cwd: root })
  runPnpmSync(['run', 'verify:m2-18:assets'], { cwd: root })
  ...
  console.log('M2-18 附件上传、持久扫描、重扫与 Web 门禁已通过。')
} finally {
