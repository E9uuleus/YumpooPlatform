...
runPnpmSync(['run', 'verify:m2-21:assets'], { cwd: root })
runPnpmSync(['run', 'check:openapi'], { cwd: root })
if (process.platform === 'win32') runSync('cmd.exe', ['/d', '/s', '/c', 'mvnw.cmd -q clean verify'], { cwd: backend })
else runSync('./mvnw', ['-q', 'clean', 'verify'], { cwd: backend })
...
console.log('M2-21 同项目 Work Item 普通关系完整门禁已通过。')
