...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m219-'))
...
const baseRef = process.env.YUMPOO_M219_BASE_REF ?? 'origin/dev'

try {
  runPnpmSync(['run', 'validate:event-contracts'], { cwd: root })
  runPnpmSync(['run', 'doc-sync'], { cwd: root })
  runPnpmSync(['run', 'verify:m2-19:assets'], { cwd: root })
  ...
  console.log('M2-19 附件下载、逻辑删除、安全维护、对账与 Web 门禁已通过。')
} finally {
