...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m211-'))
...
const baseRef=process.env.YUMPOO_M211_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-11:assets'],{cwd:root})
  ...
  console.log('M2-11 Work Item 协作字段与乐观锁完整门禁已通过。')
