...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m209-'))
...
const baseRef=process.env.YUMPOO_M209_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-09:assets'],{cwd:root})
  ...
  console.log('M2-09 Content 管理完整门禁已通过。')
