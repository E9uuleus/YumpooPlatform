...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m205-'))
...
const baseRef=process.env.YUMPOO_M205_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','verify:m2-05:assets'],{cwd:root})
  ...
  console.log('M2-05 Project 成员与唯一负责人治理完整门禁已通过。')
