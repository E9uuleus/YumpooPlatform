...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m213-'))
...
const baseRef=process.env.YUMPOO_M213_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-13:assets'],{cwd:root})
  ...
  console.log('M2-13 Table 高级查询与共享视图完整门禁已通过。')
