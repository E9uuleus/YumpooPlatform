...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m204-'))
...
const baseReference = process.env.YUMPOO_M204_BASE_REF ?? 'origin/dev'

try {
  runPnpmSync(['run', 'validate:event-contracts'], { cwd: repositoryRoot })
  runPnpmSync(['run', 'doc-sync'], { cwd: repositoryRoot })
  runPnpmSync(['run', 'verify:m2-04:assets'], { cwd: repositoryRoot })
  ...
  console.log('M2-04 Project 原子创建与初始 Content 完整门禁已通过。')
} finally {
