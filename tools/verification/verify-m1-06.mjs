...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m106-'))
...
const baseReference = process.env.YUMPOO_M106_BASE_REF ?? 'origin/dev'
...
try {
  ...
  runPnpmSync(['run', 'check:openapi-compat', '--', baselinePath], {
    cwd: repositoryRoot,
  })
  ...
  console.log('M1-06 企微登录与会话闭环验证已通过。')
} finally {
