...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m201-'))
...
const baseReference = process.env.YUMPOO_M201_BASE_REF ?? 'origin/dev'

try {
  runPnpmSync(['run', 'validate:event-contracts'], { cwd: repositoryRoot })
  runPnpmSync(['run', 'verify:m2-01:assets'], { cwd: repositoryRoot })
  ...
  console.log('M2-01 固定模板目录与版本治理完整门禁已通过。')
} finally {
