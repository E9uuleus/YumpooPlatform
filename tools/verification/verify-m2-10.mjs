...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m210-'))
...
const baseRef=process.env.YUMPOO_M210_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-10:assets'],{cwd:root})
  ...
  console.log('M2-10 Work Item 核心闭环完整门禁已通过。')
