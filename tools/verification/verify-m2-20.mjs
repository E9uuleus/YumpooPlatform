...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m220-'))
...
const baseRef = process.env.YUMPOO_M220_BASE_REF ?? 'origin/dev'

try {
  runPnpmSync(['run', 'validate:event-contracts'], { cwd: root })
  runPnpmSync(['run', 'doc-sync'], { cwd: root })
  runPnpmSync(['run', 'verify:m2-20:assets'], { cwd: root })
  ...
  console.log('M2-20 Activity 追加投影、游标查询、权限与 Web 门禁已通过。')
} finally {
