...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m202-'))
...
const baseReference = process.env.YUMPOO_M202_BASE_REF ?? 'origin/dev'

try {
  runPnpmSync(['run', 'validate:event-contracts'], { cwd: repositoryRoot })
  runPnpmSync(['run', 'doc-sync'], { cwd: repositoryRoot })
  runPnpmSync(['run', 'verify:m2-02:assets'], { cwd: repositoryRoot })
  ...
  console.log('M2-02 Workspace 生命周期完整门禁已通过。')
} finally {
