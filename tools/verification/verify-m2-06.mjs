...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m206-'))
...
const baseRef=process.env.YUMPOO_M206_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-06:assets'],{cwd:root})
  ...
  console.log('M2-06 Project 查询、激活与工作台完整门禁已通过。')
