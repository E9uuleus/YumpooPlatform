...
for(const fragment of ['activatesAllFourTypesAndRequiresCustomerOnlyOutsideProductDevelopment','retiredFrozenTemplateRemainsActivatable','queryAndWorkspaceCountsUseTheSameDatabaseVisibilityPredicate']) assert(tests.includes(fragment),`PostgreSQL 验收缺少 ${fragment}`)
...
for(const fragment of ['创建 Project','visibleProjectCount','listProjects']) assert(workbench.includes(fragment),`项目目录缺少 ${fragment}`)
