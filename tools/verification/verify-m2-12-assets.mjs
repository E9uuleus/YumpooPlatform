...
for(const forbidden of ['title=','priority=','rank=','description=','notes=','assignee_user_id=']) assert(!transitionSql.includes(forbidden),`专用状态更新不得修改 ${forbidden}`)
...
for(const fragment of ['保留未存字段草稿','必须填写说明','状态迁移 412','不显示状态迁移入口','只读 Kanban']) assert(pageTest.includes(fragment),`Web 测试缺少 ${fragment}`)
...
for(const fragment of ['Status: implemented','M2-12 状态命令','availableTransitions','Project → Content → Work Item','M2-23/M2-24']) assert(note.includes(fragment),`Work Item Agent Note 缺少 ${fragment}`)
