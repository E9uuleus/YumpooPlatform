...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m217-'))
...
const baseRef = process.env.YUMPOO_M217_BASE_REF ?? 'origin/dev'

try {
  runPnpmSync(['run', 'validate:event-contracts'], { cwd: root })
  runPnpmSync(['run', 'doc-sync'], { cwd: root })
  runPnpmSync(['run', 'verify:m2-17:assets'], { cwd: root })
  runSync(process.execPath, [
    path.join(root, 'tools', 'openapi', 'extract-openapi-baseline.mjs'),
    baseRef,
    baseline,
    metadata,
  ], { cwd: root })
  ...
  console.log('M2-17 Update 编辑、自删、治理删除与事务门禁已通过。')
} finally {
