...
const temporary=fs.mkdtempSync(path.join(os.tmpdir(),'yumpoo-m212-'))
...
const baseRef=process.env.YUMPOO_M212_BASE_REF??'origin/dev'
try{
  runPnpmSync(['run','validate:event-contracts'],{cwd:root})
  runPnpmSync(['run','doc-sync'],{cwd:root})
  runPnpmSync(['run','verify:m2-12:assets'],{cwd:root})
  ...
  console.log('M2-12 Work Item 独立状态迁移完整门禁已通过。')
