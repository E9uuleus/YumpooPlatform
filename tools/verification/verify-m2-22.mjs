...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'yumpoo-m222-'))
...
const baseRef = process.env.YUMPOO_M222_BASE_REF ?? 'origin/dev'

try {
  runPnpmSync(['run', 'validate:event-contracts'], { cwd: root })
  runPnpmSync(['run', 'doc-sync'], { cwd: root })
  runPnpmSync(['run', 'verify:m2-22:assets'], { cwd: root })
  ...
  console.log('M2-22 跨项目普通关系与不可见端占位完整门禁已通过。')
} finally {
