/* tslint:disable */
/**
 * Yumpoo API
 * Yumpoo 一期 HTTP API 的唯一静态契约源。  所有业务 operation 都必须挂载在相对 server `/api/v1` 下。M0-09 只冻结公共 schema、header、parameter、response 和 golden example；业务 paths 在对应模块 实现时按兼容新增原则加入。
 *
 * The version of the OpenAPI document: 1.0.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 *
 * @export
 * @interface Company
 */
export interface Company {
    /**
     * 不透明 UUID；客户端不得从值中推导业务语义。
     * @type {string}
     * @memberof Company
     */
    id: string;
    /**
     *
     * @type {string}
     * @memberof Company
     */
    displayName: string;
    /**
     *
     * @type {string}
     * @memberof Company
     */
    timezone: string;
    /**
     *
     * @type {string}
     * @memberof Company
     */
    weekStartDay: CompanyWeekStartDayEnum;
    /**
     *
     * @type {number}
     * @memberof Company
     */
    defaultWorkdayMinutes: number;
    /**
     *
     * @type {number}
     * @memberof Company
     */
    rowVersion: number;
}

/**
* @export
* @enum {string}
*/
export enum CompanyWeekStartDayEnum {
    Monday = 'MONDAY',
    UnknownDefaultOpenApi = '11184809'
}


/**
 * Check if a given object implements the Company interface.
 */
export function instanceOfCompany(value: object): value is Company {
    if (!('id' in value) || value['id'] === undefined) return false;
    if (!('displayName' in value) || value['displayName'] === undefined) return false;
    if (!('timezone' in value) || value['timezone'] === undefined) return false;
    if (!('weekStartDay' in value) || value['weekStartDay'] === undefined) return false;
    if (!('defaultWorkdayMinutes' in value) || value['defaultWorkdayMinutes'] === undefined) return false;
    if (!('rowVersion' in value) || value['rowVersion'] === undefined) return false;
    return true;
}

export function CompanyFromJSON(json: any): Company {
    return CompanyFromJSONTyped(json, false);
}

export function CompanyFromJSONTyped(json: any, ignoreDiscriminator: boolean): Company {
    if (json == null) {
        return json;
    }
    return {

        'id': json['id'],
        'displayName': json['displayName'],
        'timezone': json['timezone'],
        'weekStartDay': json['weekStartDay'],
        'defaultWorkdayMinutes': json['defaultWorkdayMinutes'],
        'rowVersion': json['rowVersion'],
    };
}

export function CompanyToJSON(json: any): Company {
    return CompanyToJSONTyped(json, false);
}

export function CompanyToJSONTyped(value?: Company | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {

        'id': value['id'],
        'displayName': value['displayName'],
        'timezone': value['timezone'],
        'weekStartDay': value['weekStartDay'],
        'defaultWorkdayMinutes': value['defaultWorkdayMinutes'],
        'rowVersion': value['rowVersion'],
    };
}
