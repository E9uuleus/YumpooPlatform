...
public final class AttachmentController {
    ...
    ResponseEntity<StreamingResponseBody> download(@PathVariable UUID attachmentId) {
        DownloadContent download=service.download(actors.requiredActive(),attachmentId);
    ...
    ResponseEntity<String> delete(@PathVariable UUID attachmentId,
            @Valid @RequestBody AttachmentDeleteRequest body,
        ...
        service.requireDeleteTarget(actor,attachmentId);
        long version=ifMatch.parseForVisibleResource(true,ifMatchHeader);
        UUID key=keys.parseRequired(keyHeader);
        StoredCommandResult stored=service.delete(actor,attachmentId,version,body.reason(),key,
                hasher.hash("deleteAttachment",Map.of("attachmentId",attachmentId.toString()),
                        objectMapper.valueToTree(body))).result();
        return stored(stored,HttpStatus.OK,null);
    }
