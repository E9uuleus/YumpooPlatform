...
`YUMPOO_LOCAL_AUTH_ENABLED=true` 与 `local` profile 共同启用本地身份夹具，默认保持关闭。启动时夹具经现有目录成员服务创建或刷新配置的本地管理员与内部备份管理员，经平台角色维护和命令端口授予登录账号 `APP_MANAGER`（有效权限包含 `COMPANY_ADMIN`），不建立第二套用户或权限事实。角色维护在治理锁内复用任意当前可用 APP_MANAGER；只有全公司没有可用 APP_MANAGER 时，才根据治理生命周期对内部备份管理员执行一次性首管引导或紧急恢复。
...
- 本地数据库保留真实 User、ExternalIdentity、角色、审计、事件与 Session 记录，重启可幂等复用；既有可用 APP_MANAGER 会作为授权执行者复用，已初始化但暂时缺失管理员时按正式紧急恢复语义补齐备份管理员，首管关闭规则不会被绕过。
- 该能力不能作为真实企微 OAuth、通讯录同步、扫码、HTTPS 或发布环境证据。

角色单一层级与已有公司管理员的原子升级遵循[平台角色层级决策](2026-09-25-platform-role-tiers.md)。
