...
优先级由固定枚举改为项目目录中的字符串代码且允许空；新建工作项默认不设置优先级。排序秩、筛选名称和展示颜色均从目录读取。Table、Kanban 与 Content 工作台消费同一目录和即时版本，不复制标签事实。工作项 wire 契约把优先级代码视为从目录取得并原样回传的不透明字符串，不重复声明格式约束；稳定代码格式只在标签创建和修改边界校验，避免客户端再次固化目录内部规则。OpenAPI 兼容门禁仅关闭 `incompatible.request.enum.decreased`：旧枚举变为开放代码集合会继续接受全部旧请求值，而类型、响应、正则和其他破坏性检查保持启用。
